...
var MillionaireModel = function(data,) {
    ...
    // The current money obtained
 	...
 	self.getQuestionText = function(id) {
...
$(document).ready(function() {
	$.getJSON("questions.json", function(data) {
		for(var i = 1; i <= data.games.length; i++) {
			$("#problem-set").append('<option value="' + i + '">'+ "Capitulo #"+ (i+6) + '</option>');
		}
		$("#pre-start").show();
	...
	function countdown(id){
		...
		var hoy=new Date()
		var fecha=new Date(hoy.getFullYear(),hoy.getMonth(),hoy.getDate(),(hoy.getHours()+1),hoy.getMinutes(),'00')
		//var fecha=new Date('2020','3','14','22','12','00')
		...
		if (fecha>hoy){
			...
			document.getElementById(id).innerHTML=  'Quedan: '+ horas +  ' Hora(s), ' + minutos + ' Minuto(s), ' + segundos + ' Segundo(s)'
			...
			if(minutos == 0 && segundos == 0){
				startSound('losesound', false);
				$("#game").fadeOut('slow', function() {
				$("#game-over").html('SE ACABO EL TIEMPO!!');
				$("#game-over").fadeIn('slow');
				self.transitioning = false;
				});
				}
